/*
* Modified by Adam P. Goucher to generate keypairs deterministically from
* the SHA-256 hash of a password, instead of from /dev/urandom. This means
* that a user need only remember his or her password, and not store the
* entire private-key file.
*/
...
*
* Arguments:   - unsigned char *ph: pointer to hash of password
...
* Returns 0 (success)
**************************************************/
int crypto_sign_keypair(const unsigned char *ph, unsigned char *pk, unsigned char *sk) {
  unsigned int i;
